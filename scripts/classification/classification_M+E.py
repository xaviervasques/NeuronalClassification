...
print("\n[START] Multimodal M+E→e-type baseline\n")

# ----------------------------
# 0) Load data
# ----------------------------
file_path = '../data/mee-type.csv'   # expects a column 'e-type' (Exc_1..Inh_13)
...
TECH_DROP = [
    ...
    "electrode_0_pa",        # reviewer-flagged
    "seal_gohm",             # reviewer-flagged
    "vm_for_sag",            # drop if present in export

    # Morphology (pipeline/constant; non-phenotypic)
    "neuron_reconstruction_type",  # reviewer-flagged
    "scale_factor_x",              # constant pixel scale
    "scale_factor_y",
    "scale_factor_z",

    # Extra housekeeping/flags often present
    "superseded"                   # repository/curation flag, not phenotype
]
...
# Numeric casting & impute numeric with mean
X = X.apply(pd.to_numeric, errors='ignore')
num_cols_all = X.select_dtypes(include=[np.number]).columns.tolist()
before_na = X[num_cols_all].isna().sum().sum()
X[num_cols_all] = X[num_cols_all].fillna(X[num_cols_all].mean())
after_na = X[num_cols_all].isna().sum().sum()
print(f"[NA] Missing numeric values (before→after): {before_na} → {after_na}")

# Drop constant numeric columns (safety)
...
# ----------------------------
# If you know exact onsets per protocol, set here:
...
# All numeric columns after curation
num_cols = X.select_dtypes(include=[np.number]).columns.tolist()

# Morphology columns = numeric not already in time/volt/ratio (we treat them like "other" numeric)
OTHER_COLS = [c for c in num_cols if c not in set(TIME_COLS + VOLT_COLS + RATIO_COLS)]

print(f"[COLUMNS] TIME={len(TIME_COLS)}, VOLT={len(VOLT_COLS)}, RATIO={len(RATIO_COLS)}, M-or-Other={len(OTHER_COLS)}")
if len(TIME_COLS) == 0:
...
preprocessor = ColumnTransformer(
    transformers=[
        ('time', RobustScaler(), TIME_COLS),     # preserve ms-scale deltas, robust to outliers
        ('volt', StandardScaler(), VOLT_COLS),
        ('ratio','passthrough', RATIO_COLS),     # ratios/indices already scale-free
        ('other', StandardScaler(), OTHER_COLS)  # includes morphology metrics
    ],
    remainder='drop',
    n_jobs=None
)
...
print("[SAVE] Wrote split CSVs: X_train_mee.csv, X_test_mee.csv, y_train_mee.csv, y_test_mee.csv\n")
...
# ----------------------------
# 7) Run grid (reducers × classifiers) with fixed preprocessor
# ----------------------------
results_csv = './mee-type_baseline_results.csv'
Path(results_csv).write_text('')  # reset file
print(f"[GRID] Results will be saved to: {results_csv}\n")
...
print(f"[SAVE] Wrote CV results: {results_csv} (rows={len(df_out)})\n")
...
}]).to_csv("mee-type_baseline_test_metrics.csv", index=False)
print("[SAVE] Wrote holdout metrics: mee-type_baseline_test_metrics.csv")
...
pd.concat([train_counts.rename('train'), test_counts.rename('test')], axis=1)\
  .to_csv('mee-type_class_counts.csv')
print("[SAVE] Wrote class counts: mee-type_class_counts.csv")
