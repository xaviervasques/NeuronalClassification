    ...
    if method == 'smote':
        try:
            if method == 'smote':
                ...
                smote = SMOTE(sampling_strategy={cls: n_samples for cls in np.unique(y)}, k_neighbors=2, random_state=42)
...
#methods = ['smote', 'gan', 'vae', 'flow', 'ddpm']
...
for method in methods:
    for size in sizes:
        ...
        scaler_1 = QuantileTransformer(n_quantiles=1000, output_distribution="uniform")
        ...
        data_resampled.to_csv(f'./mee-type_synthetic/mee-type_{method}_quantileUni_{size}.csv', index=False)
...
for method in methods:
    for size in sizes:
        print(f"Augmenting data using method {method} with size {size}")
        scaler_2 = QuantileTransformer(n_quantiles=1000, output_distribution="uniform")
        X_scaled_2 = scaler_2.fit_transform(X_train)
        X_resampled, y_resampled = augment_data(X_scaled_2, y_train, size, method=method)
...
for method in methods:
    for size in sizes:
        print(f"Augmenting data using method {method} with size {size}")
        scaler_3 = QuantileTransformer(n_quantiles=1000, output_distribution="uniform")
        X_scaled_3 = scaler_3.fit_transform(X_train)
        X_resampled, y_resampled = augment_data(X_scaled_3, y_train, size, method=method)
        data_resampled = pd.DataFrame(X_resampled, columns=[f'feature_{i+1}' for i in range(X_resampled.shape[1])])
        data_resampled['e-type'] = y_resampled
        data_resampled.to_csv(f'./mee-type_synthetic/mee-type_{method}_quantileUni_{size}.csv', index=False)
        print(f"Saved dataset for {method} with {size} samples per class.")
...
for method in methods:
    for size in sizes:
        print(f"Augmenting data using method {method} with size {size}")
        scaler_4 = QuantileTransformer(n_quantiles=1000, output_distribution="uniform")
...
print("Augmented datasets have been successfully saved.")
"""
"""
With reducer
"""
...
for method in methods:
    for size in sizes:
        print(f"Augmenting data using method {method} with size {size}")
        scaler_5 = QuantileTransformer(n_quantiles=1000, output_distribution="uniform")
        X_scaled_5 = scaler_5.fit_transform(X_train)
        lda = LinearDiscriminantAnalysis(n_components=2)
        X_scaled_5 = lda.fit(X_scaled_5,y_train).transform(X_scaled_5)
        X_resampled, y_resampled = augment_data(X_scaled_5, y_train, size, method=method)
        data_resampled = pd.DataFrame(X_resampled, columns=[f'feature_{i+1}' for i in range(X_resampled.shape[1])])
        data_resampled['e-type'] = y_resampled
        data_resampled.to_csv(f'./mee-type_synthetic/mee-type_{method}_quantileUni_lda_{size}.csv', index=False)
        print(f"Saved dataset for {method} with {size} samples per class.")
        
for method in methods:
    for size in sizes:
        print(f"Augmenting data using method {method} with size {size}")
        scaler_6 = QuantileTransformer(n_quantiles=1000, output_distribution="uniform")
        X_scaled_6 = scaler_6.fit_transform(X_train)
        lle = LocallyLinearEmbedding(n_components=2, method="standard")
        X_scaled_6 = lle.fit(X_scaled_6,y_train).transform(X_scaled_6)
        X_resampled, y_resampled = augment_data(X_scaled_6, y_train, size, method=method)
        data_resampled = pd.DataFrame(X_resampled, columns=[f'feature_{i+1}' for i in range(X_resampled.shape[1])])
        data_resampled['e-type'] = y_resampled
        data_resampled.to_csv(f'./mee-type_synthetic/mee-type_{method}_quantileUni_lle{size}.csv', index=False)
        print(f"Saved dataset for {method} with {size} samples per class.")
...
for method in methods:
    for size in sizes:
        print(f"Augmenting data using method {method} with size {size}")
        scaler_7 = QuantileTransformer(n_quantiles=1000, output_distribution="uniform")
        X_scaled_7 = scaler_7.fit_transform(X_train)
        lda = LinearDiscriminantAnalysis(n_components=2)
        X_scaled_7 = lda.fit(X_scaled_7,y_train).transform(X_scaled_7)
    ...
    for size in sizes:
        print(f"Augmenting data using method {method} with size {size}")
        scaler_8 = QuantileTransformer(n_quantiles=1000, output_distribution="uniform")
        X_scaled_8 = scaler_8.fit_transform(X_train)
        lle = LocallyLinearEmbedding(n_components=2, method="standard")
        X_scaled_8 = lle.fit(X_scaled_8,y_train).transform(X_scaled_8)
        X_resampled, y_resampled = augment_data(X_scaled_8, y_train, size, method=method)
        data_resampled = pd.DataFrame(X_resampled, columns=[f'feature_{i+1}' for i in range(X_resampled.shape[1])])
        data_resampled['e-type'] = y_resampled
        data_resampled.to_csv(f'./mee-type_synthetic/mee-type_{method}_quantileUni_lle_{size}.csv', index=False)
        print(f"Saved dataset for {method} with {size} samples per class.")
...
for method in methods:
    for size in sizes:
        print(f"Augmenting data using method {method} with size {size}")
        scaler_9 = QuantileTransformer(n_quantiles=1000, output_distribution="uniform")
        X_scaled_9 = scaler_9.fit_transform(X_train)
        lda = LinearDiscriminantAnalysis(n_components=2)
        X_scaled_9 = lda.fit(X_scaled_9,y_train).transform(X_scaled_9)
        X_resampled, y_resampled = augment_data(X_scaled_9, y_train, size, method=method)
        data_resampled = pd.DataFrame(X_resampled, columns=[f'feature_{i+1}' for i in range(X_resampled.shape[1])])
        data_resampled['e-type'] = y_resampled
        data_resampled.to_csv(f'./mee-type_synthetic/mee-type_{method}_quantileUni_lda_{size}.csv', index=False)
        print(f"Saved dataset for {method} with {size} samples per class.")
...
for method in methods:
    for size in sizes:
        print(f"Augmenting data using method {method} with size {size}")
        scaler_10 = QuantileTransformer(n_quantiles=1000, output_distribution="uniform")
        X_scaled_10 = scaler_10.fit_transform(X_train)
        lle = LocallyLinearEmbedding(n_components=2, method="standard")
        X_scaled_10 = lle.fit(X_scaled_10,y_train).transform(X_scaled_10)
...
for method in methods:
    for size in sizes:
        print(f"Augmenting data using method {method} with size {size}")
        scaler_11 = QuantileTransformer(n_quantiles=1000, output_distribution="uniform")
        X_scaled_11 = scaler_11.fit_transform(X_train)
        lda = LinearDiscriminantAnalysis(n_components=2)
        X_scaled_11 = lda.fit(X_scaled_11,y_train).transform(X_scaled_11)
        X_resampled, y_resampled = augment_data(X_scaled_11, y_train, size, method=method)
        data_resampled = pd.DataFrame(X_resampled, columns=[f'feature_{i+1}' for i in range(X_resampled.shape[1])])
        data_resampled['e-type'] = y_resampled
        data_resampled.to_csv(f'./mee-type_synthetic/mee-type_{method}_quantileUni_lda_{size}.csv', index=False)
        print(f"Saved dataset for {method} with {size} samples per class.")
...
for method in methods:
    for size in sizes:
        print(f"Augmenting data using method {method} with size {size}")
        scaler_12 = QuantileTransformer(n_quantiles=1000, output_distribution="uniform")
        X_scaled_12 = scaler_12.fit_transform(X_train)
        lle = LocallyLinearEmbedding(n_components=2, method="standard")
        X_scaled_12 = lle.fit(X_scaled_12,y_train).transform(X_scaled_12)
        X_resampled, y_resampled = augment_data(X_scaled_12, y_train, size, method=method)
        data_resampled = pd.DataFrame(X_resampled, columns=[f'feature_{i+1}' for i in range(X_resampled.shape[1])])
        data_resampled['e-type'] = y_resampled
        data_resampled.to_csv(f'./mee-type_synthetic/mee-type_{method}_quantileUni_lle_{size}.csv', index=False)
        print(f"Saved dataset for {method} with {size} samples per class.")
