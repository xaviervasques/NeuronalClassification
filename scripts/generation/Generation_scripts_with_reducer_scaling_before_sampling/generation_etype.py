...
for method in methods:
    for size in sizes:
        ...
        scaler_1 = QuantileTransformer(n_quantiles=1000, output_distribution="normal")
        ...
        data_resampled.to_csv(f'./e-type_synthetic/e-type_{method}_quantile_{size}.csv', index=False)
...
for method in methods:
    for size in sizes:
        ...
        data_resampled = pd.DataFrame(X_resampled, columns=[f'feature_{i+1}' for i in range(X_resampled.shape[1])])
        data_resampled['e-type'] = y_resampled
        data_resampled.to_csv(f'./e-type_synthetic/e-type_{method}_maxabs_{size}.csv', index=False)
        print(f"Saved dataset for {method} with {size} samples per class.")
...
for method in methods:
    for size in sizes:
        ...
        data_resampled = pd.DataFrame(X_resampled, columns=[f'feature_{i+1}' for i in range(X_resampled.shape[1])])
        data_resampled['e-type'] = y_resampled
        data_resampled.to_csv(f'./e-type_synthetic/e-type_{method}_robust_{size}.csv', index=False)
        print(f"Saved dataset for {method} with {size} samples per class.")
...
for method in methods:
    for size in sizes:
        print(f"Augmenting data using method {method} with size {size}")
        scaler_4 = QuantileTransformer(n_quantiles=1000, output_distribution="normal")
...
for method in methods:
    for size in sizes:
        print(f"Augmenting data using method {method} with size {size}")
        scaler_7 = QuantileTransformer(n_quantiles=1000, output_distribution="normal")
...
for method in methods:
    for size in sizes:
        print(f"Augmenting data using method {method} with size {size}")
        scaler_10 = QuantileTransformer(n_quantiles=1000, output_distribution="normal")
...
for method in methods:
    for size in sizes:
        ...
        X_scaled_1 = nca.fit(X_scaled_1,y_train).transform(X_scaled_1)
        X_resampled, y_resampled = augment_data(X_scaled_1, y_train, size, method=method)
        data_resampled = pd.DataFrame(X_resampled, columns=[f'feature_{i+1}' for i in range(X_resampled.shape[1])])
        data_resampled['e-type'] = y_resampled
        data_resampled.to_csv(f'./e-type_synthetic/e-type_{method}_quantile_nca_{size}.csv', index=False)
        print(f"Saved dataset for {method} with {size} samples per class.")
...
for method in methods:
    for size in sizes:
        ...
        X_scaled_2 = lda.fit(X_scaled_2, y_train).transform(X_scaled_2)
        X_resampled, y_resampled = augment_data(X_scaled_2, y_train, size, method=method)
        data_resampled = pd.DataFrame(X_resampled, columns=[f'feature_{i+1}' for i in range(X_resampled.shape[1])])
        data_resampled['e-type'] = y_resampled
        data_resampled.to_csv(f'./e-type_synthetic/e-type_{method}_maxabs_lda_{size}.csv', index=False)
        print(f"Saved dataset for {method} with {size} samples per class.")
...
for method in methods:
    for size in sizes:
        ...
        scaler_3 = RobustScaler()
        X_scaled_3 = scaler_3.fit_transform(X_train)
        X_scaled_3 = lda.fit(X_scaled_3, y_train).transform(X_scaled_3)
        X_resampled, y_resampled = augment_data(X_scaled_3, y_train, size, method=method)
        data_resampled = pd.DataFrame(X_resampled, columns=[f'feature_{i+1}' for i in range(X_resampled.shape[1])])
        data_resampled['e-type'] = y_resampled
        data_resampled.to_csv(f'./e-type_synthetic/e-type_{method}_robust_lda_{size}.csv', index=False)
        print(f"Saved dataset for {method} with {size} samples per class.")
...
for method in methods:
    for size in sizes:
        ...
        X_scaled_4 = scaler_4.fit_transform(X_train)
        nca = NeighborhoodComponentsAnalysis(n_components=2, init="pca")
        X_scaled_4 = nca.fit(X_scaled_4,y_train).transform(X_scaled_4)
        X_resampled, y_resampled = augment_data(X_scaled_4, y_train, size, method=method)
        data_resampled = pd.DataFrame(X_resampled, columns=[f'feature_{i+1}' for i in range(X_resampled.shape[1])])
        data_resampled['e-type'] = y_resampled
        data_resampled.to_csv(f'./e-type_synthetic/e-type_{method}_quantile_nca_{size}.csv', index=False)
...
for method in methods:
    for size in sizes:
        ...
        scaler_6 = RobustScaler()
        X_scaled_6 = scaler_6.fit_transform(X_train)
        X_scaled_6 = lda.fit(X_scaled_6, y_train).transform(X_scaled_6)
        X_resampled, y_resampled = augment_data(X_scaled_6, y_train, size, method=method)
        data_resampled = pd.DataFrame(X_resampled, columns=[f'feature_{i+1}' for i in range(X_resampled.shape[1])])
        data_resampled['e-type'] = y_resampled
        data_resampled.to_csv(f'./e-type_synthetic/e-type_{method}_robust_lda_{size}.csv', index=False)
...
for method in methods:
    for size in sizes:
        ...
        X_scaled_7 = scaler_7.fit_transform(X_train)
        nca = NeighborhoodComponentsAnalysis(n_components=2, init="pca")
        X_scaled_7 = nca.fit(X_scaled_7,y_train).transform(X_scaled_7)
        X_resampled, y_resampled = augment_data(X_scaled_7, y_train, size, method=method)
        data_resampled = pd.DataFrame(X_resampled, columns=[f'feature_{i+1}' for i in range(X_resampled.shape[1])])
        data_resampled['e-type'] = y_resampled
        data_resampled.to_csv(f'./e-type_synthetic/e-type_{method}_quantile_nca_{size}.csv', index=False)
...
for method in methods:
    for size in sizes:
        ...
        scaler_8 = MaxAbsScaler()
        X_scaled_8 = scaler_8.fit_transform(X_train)
        X_scaled_8 = lda.fit(X_scaled_8, y_train).transform(X_scaled_8)
        X_resampled, y_resampled = augment_data(X_scaled_8, y_train, size, method=method)
        data_resampled = pd.DataFrame(X_resampled, columns=[f'feature_{i+1}' for i in range(X_resampled.shape[1])])
        data_resampled['e-type'] = y_resampled
        data_resampled.to_csv(f'./e-type_synthetic/e-type_{method}_maxabs_lda_{size}.csv', index=False)
...
"""
methods = ['gan']
sizes = [100]  # Example sizes to augment data to
"""
for method in methods:
    for size in sizes:
        print(f"Augmenting data using method {method} with size {size}")
        ...
        X_scaled_10 = scaler_10.fit_transform(X_train)
        nca = NeighborhoodComponentsAnalysis(n_components=2, init="pca")
        X_scaled_10 = nca.fit(X_scaled_10,y_train).transform(X_scaled_10)
        X_resampled, y_resampled = augment_data(X_scaled_10, y_train, size, method=method)
        data_resampled = pd.DataFrame(X_resampled, columns=[f'feature_{i+1}' for i in range(X_resampled.shape[1])])
        data_resampled['e-type'] = y_resampled
        data_resampled.to_csv(f'./e-type_synthetic/e-type_{method}_quantile_nca_{size}.csv', index=False)
...
for method in methods:
    for size in sizes:
        ...
        scaler_12 = RobustScaler()
        X_scaled_12 = scaler_12.fit_transform(X_train)
        X_scaled_12 = lda.fit(X_scaled_12, y_train).transform(X_scaled_12)
        X_resampled, y_resampled = augment_data(X_scaled_12, y_train, size, method=method)
        data_resampled = pd.DataFrame(X_resampled, columns=[f'feature_{i+1}' for i in range(X_resampled.shape[1])])
        data_resampled['e-type'] = y_resampled
        data_resampled.to_csv(f'./e-type_synthetic/e-type_{method}_robust_lda_{size}.csv', index=False)
